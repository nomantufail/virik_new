            ...
            <table class="table" style="margin-top: 15px; width: 100%;">
                ...
                <tr style="border-bottom: 1px solid lightgray;">
                    <td colspan="3" style="width: ;"><b>Route</b><br>
                        <select name="route_<?= $row_counter ?>" onchange="i_think_route_changed(0,<?= $row_counter?>)" class="route route_select" style="width: 100%;">
                            <?php
                            $selected = '';
                            foreach($local_self_routes as $route){
                                if($trip[$row_counter-1]->source == $route->sourceId &&
                                    $trip[$row_counter-1]->destination == $route->destinationId &&
                                        $trip[$row_counter-1]->product == $route->productId)
                                {
                                    $selected = 'selected';
                                }else{
                                    $selected = '';
                                ...
                                ?>
                                <option <?= $selected ?> value="<?= $route->id; ?>"><?= $route->source." To ".$route->destination." ( ".$route->product." )"; ?></option>
                            <?php
                            }
                            ?>
                        </select>
                    ...
                    <td>
                        ...
                        <input id="company_freight_unit_<?= $row_counter ?>" name="company_freight_unit_<?= $row_counter ?>" value="<?= $trip[$row_counter - 1]->company_freight_unit ?>" <?= (($row_counter < 2)?"required='required'":'') ?> class="company_freight_unit_1" style="width: 80px;" type="number" step="any">
                    </td>
                </tr>
                <tr style="border-bottom: 1px solid lightgray;">
                    <td colspan="2">
            ...
            <table class="table" style="margin-top: 15px; width: 100%;">
                <tr style="border-bottom: 1px solid lightgray;">
                    <td colspan="3" style="width: ;"><b>Route</b><br>
                        <select name="route_<?= $row_counter ?>" onchange="i_think_route_changed(0,<?= $row_counter?>)" class="route route_select" style="width: 100%;">
                            <?php
                            foreach($local_routes as $route){
                                ?>
                                <option value="<?= $route->id; ?>"><?= $route->source." To ".$route->destination." ( ".$route->product." )"; ?></option>
                            <?php
                            }
                            ?>
                        </select>
