...
function take_care_of_product_info_and_commissions_data(callback){
    ...
    fetch_customer_contractor_commission(customer_id, contractor_id, function(result) {
        ...
        remaining_for_customer_area.innerHTML = limit_number((100 - contractor_commission_input.value));
        ...
        for(var count = 0; count<(pannel_count.value-1); count++){
            ...
            contractor_commission_rs.innerHTML = limit_number(contractor_commission_input.value * (((freight_unit_input.value !='')?freight_unit_input.value:0) * initial_quantity_input.value)/100);
            //setting remaining for customer rupees in products info area..
            remaining_for_customer_rs.innerHTML = limit_number(((((freight_unit_input.value !='')?freight_unit_input.value:0) * initial_quantity_input.value) - contractor_commission_rs.innerHTML));
        ...
        fetch_company_contractor_commission(company_id, contractor_id, function(result) {
            ...
            for(var count = 0; count<(pannel_count.value-1); count++){
                ...
                company_commission_1_rs.innerHTML = limit_number(company_commission_1_input.value * (((freight_unit_input.value !='')?freight_unit_input.value:0) * initial_quantity_input.value)/100);
                company_wht_rs_1.innerHTML = limit_number(wht_input.value * (((freight_unit_input.value !='')?freight_unit_input.value:0) * initial_quantity_input.value)/100);
...
function i_think_route_changed(count,section){
    ...
    var route_id = document.getElementsByClassName("route")[count].options[route_index].value;

    fetch_source_destination( route_id, function(route_detail){
        source_id = route_detail.source;
        destination_id = route_detail.destination;
        product_id = route_detail.product;
...
function display_row(){
    ...
    ?>
    i_think_route_changed(<?= $route_type ?>,pannel_count.value-1);
}
...
function set_quantites_and_amounts_on_first_time_load(count,section){
    ...
    if(section === undefined){
        ...
        ?>
        i_think_route_changed(<?= $route_type ?>, count);
    }
