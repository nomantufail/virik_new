...
//        $this->ignoreShortageAmountDeductionVouchers();
    ...
    public function makeFreightOnShortageVouchersEntriesArray()
    {
        ...
        $freight_on_shortage_qty_cmp = [];
        foreach($manage_account_black_oil as $ma){
            $freight_on_shortage_qty_cmp[$ma->trip_detail_id] = round($ma->freight_on_shortage_qty_cmp, 2);
        }
        ...
        foreach($saved_vouchers_data as $voucher_data)
        {
            ...
            //calculating amount
            $amount = $freight_on_shortage_qty_cmp[$voucher_data->trip_product_detail_id];
